...
var ParamsInterceptor = func(ctx context.Context, req interface{}, handler HandleFunc) (reply interface{}, err error) {
	if pi, ok := req.(interface {
		SetParams(ctx context.Context) error
	}); ok {
		if err = pi.SetParams(ctx); err != nil {
			return
		}
	}
	return handler(ctx, req)
